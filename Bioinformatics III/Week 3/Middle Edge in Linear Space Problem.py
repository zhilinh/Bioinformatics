...
for i in range(1, len(seq1) + 1):
    for j in range(1, len(seq2) // 2 + 1):
        s[i].append(max(s[i][j - 1] - 5, s[i - 1][j] - 5, s[i - 1][j - 1] + blosum[seq1[i - 1]][seq2[j - 1]]))
for i in range(len(seq1) + 1):
    s_mid.append(s[i][len(seq2) // 2])
...
for i in range(len(seq1)):
    s.append([-5 * (i + 1)])
    backtrack.append(['up'])
for j in range(len(seq2) - len(seq2) // 2):
    s[0].append(-5 * (j + 1))
...
for i in range(1, len(seq1) + 1):
    for j in range(1, len(seq2) // 2 + 1):
        s[i].append(max(s[i][j - 1] - 5, s[i - 1][j] - 5, s[i - 1][j - 1] + blosum[seq1[i - 1]][seq2[j - 1]]))
        if s[i][j] == s[i][j - 1] - 5:
            backtrack[i].append('left')
        elif s[i][j] == s[i - 1][j] - 5:
            backtrack[i].append('up')
        else:
            backtrack[i].append('upleft')
max_val = float('-inf')
for i in range(len(seq1) + 1):
    s_mid[i] += s[- i - 1][len(seq2) // 2]
    if s_mid[i] > max_val:
        max_val = s_mid[i]
        i_mid = i
if backtrack[- i_mid - 1][len(seq2) // 2] == 'up':
    print('(' + str(i_mid) + ', ' + str(len(seq2) // 2) + ') (' + str(i_mid + 1) + ', ' + str(len(seq2) // 2) + ')')
elif backtrack[- i_mid - 1][len(seq2) // 2] == 'left':
    print('(' + str(i_mid) + ', ' + str(len(seq2) // 2) + ') (' + str(i_mid) + ', ' + str(len(seq2) // 2 + 1) + ')')
else:
    print('(' + str(i_mid) + ', ' + str(len(seq2) // 2) + ') (' + str(i_mid + 1) + ', ' + str(len(seq2) // 2 + 1) + ')')